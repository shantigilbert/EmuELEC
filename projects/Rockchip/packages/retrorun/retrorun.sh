...
# Copyright (C) 2021-2022 AmberELEC (https://github.com/AmberELEC)
# Copyright (C) 2022-present Josh L (https://github.com/Langerz82)
...
if [ ! -f ${RRCONF} ]; then
  cp -f /usr/config/retrorun/configs/retrorun.cfg /storage/.config/retrorun/configs/retrorun.cfg
  chmod 664 /storage/.config/retrorun/configs/retrorun.cfg
fi
...
sleep 0.2
if [[ "$1" == "32" ]] || [[ "$2" =~ "pcsx_rearmed" ]] || [[ "$2" =~ "parallel_n64" ]]
then
    echo 'using 32bit'
