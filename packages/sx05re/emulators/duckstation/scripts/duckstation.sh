...
#Check if textures dirctory exits if not create symlink to link textures folder in /storage/roms/psx/textures to enable texture replacement
if [ ! -d "${CONFIG_DIR}/textures" ]; then
    ln -s /storage/roms/psx/textures "${CONFIG_DIR}/textures"
fi

#if texture folder exists remove it and create symlink to link textures folder in /storage/roms/psx/textures to enable texture replacement
